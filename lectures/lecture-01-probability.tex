...
\section{A compact probability dictionary}

\begin{center}
\begin{tabularx}{\textwidth}{@{}L{0.31\textwidth}L{0.25\textwidth}Y@{}}
\toprule
\textbf{Question} & \textbf{Object} & \textbf{Operation} \\
\midrule
Which outcomes are possible? & Sample space \(\Omega\) & Specify the model \\
Did a statement occur? & Event \(A\subseteq\Omega\) & Use set operations \\
What changes after learning \(B\)? & \(\Prob(A\mid B)\) & Restrict and renormalize \\
What are all routes to evidence? & Total probability & Sum weighted routes \\
What caused observed evidence? & Bayes' theorem & Reverse the conditional direction \\
Probability at a discrete value & PMF \(p_X(x)\) & Sum masses \\
Probability over a continuous interval & PDF \(f_X(x)\) & Integrate area \\
Probability accumulated to the left & CDF \(F_X(x)\) & Evaluate or subtract CDF values \\
\bottomrule
\end{tabularx}
\end{center}
...
\]
Find \(a\), write the CDF, and compute \(\Prob(-1<X\le2)\).
\end{exercise}
...
\[
  ...
  \end{cases}
\]
Find \(c\), derive \(F_X\), and calculate \(\Prob(0.2<X\le0.7)\).
\end{exercise}
...
\begin{enumerate}
  ...
  \[
    ...
    \begin{cases}
      ...
      1, & x\ge2;
    \end{cases}
    \qquad \Prob(-1<X\le2)=\frac34.
  \]
  \item \(c=2\), \(F_X(x)=x^2\) on \([0,1]\), and
  \(\Prob(0.2<X\le0.7)=0.45\).
\end{enumerate}
